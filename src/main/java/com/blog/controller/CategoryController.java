...
public class CategoryController extends BaseController {
    ...
    public ResponseEntity<Page<Category>> queryByPage(Category category, PageRequest pageRequest) {
     ...
     @PostMapping("/getall")
    public String queryall(Category category, @RequestParam("page") int offset, @RequestParam("pageSize")int pageSize){
        List<Category> Categorys = this.categoryService.queryAllByLimits(category,offset,pageSize);
        long counts = categoryService.count(category);
